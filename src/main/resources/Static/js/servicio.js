/**
 * Haven Glam - servicio.js
 * Validaciones dinámicas por tipo de campo y previsualización de imágenes para Servicios
 */
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (form) {
        const nombreInput = form.querySelector('#nombreServicio');
        const descripcionTextarea = form.querySelector('#descripcion');
        const precioInput = form.querySelector('#precio');
        const duracionInput = form.querySelector('#duracionMinutos');
        ...
        // ---------- 3. Previsualización y validación de archivo de Imagen ----------
        if (fileInput) {
        ...
        // ---------- 4. Limpieza de errores al interactuar con campos ----------
        const allFields = [nombreInput, descripcionTextarea, precioInput, duracionInput, categoriaSelect, estadoSelect];
        ...
        // ---------- 5. Validación completa al enviar el formulario ----------
        form.addEventListener('submit', function (event) {
            let isValid = true;
            clearAllErrors(form);

            // Validar Nombre de Servicio (Texto obligatorio, 3-50 caracteres, no solo espacios)
            if (nombreInput) {
                const nombre = nombreInput.value.trim();
                if (nombre.length === 0) {
                    showFieldError(nombreInput, 'El nombre del servicio es obligatorio.');
                    isValid = false;
            ...
            // Validar Descripción (Opcional, pero máx 200)
            ...
            // Validar Duración (Time mayor a 00:00)
            if (duracionInput) {
                const duracionVal = duracionInput.value.trim();
                if (!duracionVal) {
                    showFieldError(duracionInput, 'La duración estimada es obligatoria.');
                    isValid = false;
                } else if (duracionVal === '00:00') {
                    showFieldError(duracionInput, 'La duración del servicio debe ser mayor a 0 minutos.');
                    isValid = false;
                }
            }
